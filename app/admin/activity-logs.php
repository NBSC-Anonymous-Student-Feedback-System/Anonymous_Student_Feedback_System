...
<head>
  ...
  <title>Activity Logs — NBSC Feedback</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .user-email { font-size: 12px; color: #6b7280; margin-top: 1px; }
    .action-tag {
      font-family: 'DM Mono', monospace;
      font-size: 11.5px; background: #f3f4f6;
      padding: 2px 8px; border-radius: 5px; color: #374151;
      white-space: nowrap;
    }
    .desc-cell  { max-width: 260px; font-size: 13px; color: #374151; }
    .ip-cell    { font-family: 'DM Mono', monospace; font-size: 12px; color: #6b7280; white-space: nowrap; }
    .time-cell  { font-size: 12px; color: #9ca3af; white-space: nowrap; }
    .id-cell    { font-size: 12px; color: #9ca3af; }
...
<div class="hamburger-menu" id="hamburgerMenu">
  ...
  <a href="<?= BASE_URL ?>/app/admin/activity-logs.php" class="menu-link active">
...
<div class="page-wrap">
  ...
  <div class="table-card">
    <div class="table-card-header">
      <span class="table-card-title">All Logs</span>
      <span style="font-size:12px;color:#6b7280;"><?= count($logs) ?> entries</span>
    </div>
    <div style="overflow-x:auto;">
      <table>
        ...
        <tbody>
          <?php if (empty($logs)): ?>
            <tr class="empty-row"><td colspan="7">No logs found.</td></tr>
          <?php else: foreach ($logs as $log): ?>
            <tr>
              <td class="id-cell"><?= $log['log_id'] ?></td>
              <td>
                <div class="user-name"><?= sanitize($log['full_name']) ?></div>
                <div class="user-email"><?= sanitize($log['email']) ?></div>
              </td>
              <td><?= roleBadge($log['role']) ?></td>
              <td><span class="action-tag"><?= sanitize($log['action']) ?></span></td>
              <td class="desc-cell"><?= sanitize($log['description']) ?></td>
              <td class="ip-cell"><?= sanitize($log['ip_address']) ?></td>
              <td class="time-cell"><?= timeAgo($log['created_at']) ?></td>
            </tr>
