...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    $fid=(int)$_POST['feedback_id']; $status=$_POST['new_status']; $notes=trim($_POST['review_notes']??'');
    if (in_array($status,['pending','reviewed','resolved'])) {
        ...
        logActivity($pdo,$_SESSION['user_id'],'REVIEW_FEEDBACK',"Staff reviewed feedback #$fid → $status");
        $msg='Feedback updated.';
    }
}

$fs=$_GET['status']??''; $fp=$_GET['priority']??'';
$where=[]; $params=[];
...
renderHeader('Feedback – Staff');
renderSidebar('staff','All Feedback');
?>
<div class="topbar"><span class="topbar-title">All Feedback</span></div>
<div class="content">
  <div class="page-header"><h1>Feedback</h1><p>Review and update feedback status.</p></div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><button class="btn btn-outline btn-sm" onclick="openReview(<?= $fb['feedback_id'] ?>,'<?= addslashes(sanitize($fb['message'])) ?>','<?= $fb['status'] ?>','<?= $fb['priority'] ?>','<?= $fb['category'] ?>')">Review</button></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
...
<div id="reviewModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:200;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:12px;width:520px;max-width:90vw;box-shadow:0 20px 60px rgba(0,0,0,0.15);">
    ...
    <div style="padding:20px 24px;">
      <div style="margin-bottom:14px;"><div style="font-size:12px;color:var(--text-muted);margin-bottom:4px;">Category / Priority</div><div id="modal-meta"></div></div>
      ...
      <form method="POST">
        ...
        <div class="form-group"><label class="form-label">Update Status</label><select name="new_status" id="modal-status" class="form-control"><option value="pending">Pending</option><option value="reviewed">Reviewed</option><option value="resolved">Resolved</option></select></div>
        <div class="form-group"><label class="form-label">Review Notes</label><textarea name="review_notes" class="form-control" rows="3" placeholder="Add notes..."></textarea></div>
